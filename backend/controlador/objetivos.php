...
switch ($metodo) {
    case 'GET':
        if (isset($_GET['id_cliente'])) {
            $id = $_GET['id_cliente'];
            echo json_encode($objetivos->filtrarIdCliente($id));
        } else {
            echo json_encode($objetivos->consultar());
        }
        break;

    case 'POST':
        $datos = json_decode(file_get_contents("php://input"));
        echo json_encode($objetivos->insertar($datos));
        break;
    ...
    case 'PUT':
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            $datos = json_decode(file_get_contents("php://input"));
            echo json_encode($objetivos->editar($datos, $id));
        } else {
    ...
    case 'DELETE':
        if (isset($_GET['id'])) {
            $id = $_GET['id'];
            echo json_encode($objetivos->eliminar($id));
        } else {
