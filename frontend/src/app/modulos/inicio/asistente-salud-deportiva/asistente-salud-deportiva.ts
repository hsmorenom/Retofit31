...
@Component({
  selector: 'app-asistente-salud-deportiva',
  standalone: true,
  imports: [CommonModule],
  templateUrl: './asistente-salud-deportiva.html'
})
export class AsistenteSaludDeportiva implements OnInit {
...
cargarClientesSinAntropometricos() {
  ...
  this.eventosService.consultar().subscribe({
    next: (data: any[]) => {
      ...
      this.eventosProximos = data.filter(e => {
        const f = new Date(e.FECHA_ACTIVIDAD);
        f.setHours(0,0,0,0);
        return f >= hoy;
      });
